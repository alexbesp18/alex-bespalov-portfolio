...
CONFIG_FILE = Path("stock_options_wx_v3.jsonl")
...
def paint_black(ctrl: wx.Window):
...
class StockOptionsFrame(wx.Frame):
    def __init__(self, parent=None):
        ...
        self.rows = []  # list of widget dicts
        self.load_config()
        self.Centre()
        self.Show()
    ...
    def _build_header(self):
        pnl = wx.Panel(self.panel); pnl.SetBackgroundColour(WHITE)
        hbox = wx.BoxSizer(wx.HORIZONTAL)
        title = wx.StaticText(pnl, label=APP_NAME)
        title.SetFont(wx.Font(16 if self.is_mac else 14, wx.FONTFAMILY_DEFAULT,
                              wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD))
        title.SetForegroundColour(BLACK)

        save_btn = wx.Button(pnl, label="Save Changes")
        if self.is_mac:
            save_btn.SetBackgroundColour(BLUE_MAC); save_btn.SetForegroundColour(WHITE)
    ...
    def _build_column_headers(self):
        pnl = wx.Panel(self.panel); pnl.SetBackgroundColour(GRAY_BAR)
        hbox = wx.BoxSizer(wx.HORIZONTAL)

        def add_header(text, width):
            lbl = wx.StaticText(pnl, label=text)
            lbl.SetFont(wx.Font(11 if self.is_mac else 10, wx.FONTFAMILY_DEFAULT,
                                 wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD))
    ...
    def _build_rows_container(self):
        self.rows_panel = wx.Panel(self.panel); self.rows_panel.SetBackgroundColour(WHITE)
        self.rows_sizer = wx.BoxSizer(wx.VERTICAL)
        self.rows_panel.SetSizer(self.rows_sizer)
        self.vbox.Add(self.rows_panel, 1, wx.EXPAND | wx.LEFT | wx.RIGHT | wx.TOP, 10)

    # ── Config I/O ────────────────────────────────────────────────────────────

    def load_config(self):
        self.rows_sizer.Clear(delete_windows=True); self.rows.clear()
        if CONFIG_FILE.exists():
            for line in CONFIG_FILE.read_text().splitlines():
                if line.strip():
                    self.add_row(data=json.loads(line))
            self.SetStatusText(f"Loaded {len(self.rows)} row(s) from {CONFIG_FILE}")
        else:
            self.add_row()
            self.SetStatusText("No config file – starting fresh.")
        self._refresh_rows()

    def save_config(self, _evt=None):
        data_out = []
        for idx, w in enumerate(self.rows, 1):
            ticker = w["ticker"].GetValue().strip()
            if not ticker:
                self.SetStatusText(f"Row {idx}: ticker cannot be empty"); return
            if w["start"].GetValue() > w["end"].GetValue():
                self.SetStatusText(f"Row {idx}: start date after end date"); return
            data_out.append({
    ...
    def add_row(self, idx=-1, data=None):
        ...
        if idx == -1: idx = len(self.rows)
        self.rows_sizer.Insert(idx, w["panel"], 0, wx.EXPAND | wx.BOTTOM, 2)
        self.rows.insert(idx, w)
        self._refresh_rows(); self.panel.Layout(); self.panel.FitInside()

    def _delete_row(self, w):
        self.rows.remove(w); w["panel"].Destroy()
        self._refresh_rows(); self.panel.Layout(); self.panel.FitInside()
    ...
    def _refresh_rows(self):
        ...
        for i, w in enumerate(self.rows):
            ...
            for c in w["panel"].GetChildren():
                if not isinstance(c, (wx.Button, wx.Choice, wx.adv.DatePickerCtrl, wx.SpinCtrlDouble)):
                    c.SetBackgroundColour(bg)

    # ── Row factory ──────────────────────────────────────────────────────────

    def _make_row_widgets(self):
        p = wx.Panel(self.rows_panel); h = wx.BoxSizer(wx.HORIZONTAL)
        months = ["January","February","March","April","May","June","July","August","September","October","November","December"]

        plus = wx.Button(p, label="+", size=(40,25), style=wx.BORDER_NONE)
        plus.SetBackgroundColour(GREEN_BG); plus.SetForegroundColour(WHITE)

        chk  = wx.CheckBox(p)
        ticker = wx.TextCtrl(p, size=(COLS["ticker"], -1)); paint_black(ticker)
        price  = wx.SpinCtrlDouble(p, value="100.00", min=0.0, max=1e5,
                                   inc=0.01, size=(COLS["price"], -1)); price.SetDigits(2); paint_black(price)
        typ    = wx.Choice(p, choices=["Call","Put"], size=(COLS["type"], -1)); typ.SetSelection(0); paint_black(typ)
        month  = wx.Choice(p, choices=months, size=(COLS["month"], -1)); month.SetSelection(0); paint_black(month)
        year   = wx.TextCtrl(p, value=str(datetime.datetime.now().year), size=(COLS["year"], -1)); paint_black(year)

        sz = (COLS["date"], -1)
        DateCtrl = wx.adv.GenericDatePickerCtrl if self.is_mac else wx.adv.DatePickerCtrl
        start = DateCtrl(p, size=sz); paint_black(start)
        end   = DateCtrl(p, size=sz); paint_black(end)
        end_dt = wx.DateTime.Now(); end_dt.Add(wx.DateSpan(days=30)); end.SetValue(end_dt)

        w = dict(panel=p, chk=chk, ticker=ticker, price=price, type=typ,
                 month=month, year=year, start=start, end=end)

        delete = wx.Button(p, label="×", size=(40,25), style=wx.BORDER_NONE)
        delete.SetBackgroundColour(RED_BG); delete.SetForegroundColour(WHITE)
        delete.Bind(wx.EVT_BUTTON, lambda e: self._delete_row(w))
        w["del"] = delete

        plus.Bind(wx.EVT_BUTTON, lambda e: self.add_row(self.rows.index(w)+1))

        def pad(ctrl): h.Add(ctrl, 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 5)
        h.Add(plus, 0, wx.ALIGN_CENTER_VERTICAL | wx.LEFT | wx.RIGHT, 5)
        h.Add(chk, 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 5)
        for ctrl in (ticker, price, typ, month, year, start, end, delete): pad(ctrl)
        p.SetSizer(h)
        return w
