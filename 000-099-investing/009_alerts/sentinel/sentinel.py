...
def main():
    ...
    parser = argparse.ArgumentParser(description="Sentinel Daily Scan")
    ...
    if args.reminder:
        ...
        subject = f"[SENTINEL] REMINDER — {buy_count} BUY, {sell_count} SELL — {digest_id}"
    ...
    if results:
        ...
        subject = f"[SENTINEL] {buy_count} BUY, {sell_count} SELL — {datetime.now().strftime('%b %d')}"
