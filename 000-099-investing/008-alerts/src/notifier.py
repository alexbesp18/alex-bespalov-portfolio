...
class Notifier:
    ...
    def format_email_body(self, results: List[Dict], matrix_data: Optional[List[Dict]] = None, mode: str = "main"):
        ...
        body = "<h1 style='margin-bottom: 6px;'>Stock Alerts</h1>"
        ...
        body += (
            ...
            "<code>python main.py --archive SYMBOL</code> (or <code>SYMBOL:TRIGGER_KEY</code>)."
            "</p>"
        )
    ...
    def format_subject(self, buy_count: int, sell_count: int, mode: str = "main") -> str:
        ...
        if mode == "reminder":
            return f"Reminder: Stock Signals - {date_str}"
        ...
        if parts:
            return f"Stock Signals: {', '.join(parts)} - {date_str}"
        else:
            return f"Stock Signals: No Triggers - {date_str}"
