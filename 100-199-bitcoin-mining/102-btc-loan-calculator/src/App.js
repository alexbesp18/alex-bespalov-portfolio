...
// Wrappers to match original API (shared-core uses different signatures)
const formatCurrency = (value, decimals = 2) => formatCurrencyCore(value, { decimals });
const formatBTC = (value, decimals = 6) => {
  const num = parseFloat(value);
  if (isNaN(num)) return '0.000000 BTC';
  return `${num.toFixed(decimals)} BTC`;
};
const formatPercent = (value, decimals = 1) => formatDecimalAsPercentage(value, decimals);
